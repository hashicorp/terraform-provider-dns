	...
	if meta != nil {
		...
		if d.HasChange("addresses") {
			...
			for _, addr := range remove {
				rr_remove, err := dns.NewRR(fmt.Sprintf("%s %d AAAA %s", rec_fqdn, ttl, stripLeadingZeros(addr.(string))))
				if err != nil {
			...
			for _, addr := range add {
				rr_insert, err := dns.NewRR(fmt.Sprintf("%s %d AAAA %s", rec_fqdn, ttl, stripLeadingZeros(addr.(string))))
				if err != nil {
					return fmt.Errorf("error reading DNS record: %s", err)
				}
